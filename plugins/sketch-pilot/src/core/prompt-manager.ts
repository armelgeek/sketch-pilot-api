...
export class PromptManager {
  ...
  buildImagePrompt(
    ...
    // ── Assemble ──────────────────────────────────────────────────────────
    // Simplified: only the person's action/emotion and the location.
    const simpleAction = `${subjectPart}${expressionPart}`.trim()
    const simpleLocation = sanitizedBg ? `location: ${sanitizedBg}` : ''
    const prompt = [simpleAction, simpleLocation]
