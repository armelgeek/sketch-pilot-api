/**
 * Polyfills for the sketch-pilot plugin.
 * Ensures that environment-specific globals like DecompressionStream are available.
 */
...
const targets = [globalThis, global] as any[]

targets.forEach((target) => {
  if (target && typeof target.DecompressionStream === 'undefined') {
    try {
      target.DecompressionStream = CustomDecompressionStream
      console.log(
        `[SketchPilot] DecompressionStream polyfill applied to ${target === globalThis ? 'globalThis' : 'global'}`
      )
    } catch {
      // Ignore errors if we can't write to one of the targets
    }
  }
})
