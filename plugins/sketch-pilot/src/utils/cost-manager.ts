import { QualityMode, VideoGenerationOptions } from '../types/video-script.types';
...
export class CostManager {
    static calculateVideoCost(options: VideoGenerationOptions, sceneCount: number): number {
    ...
    static getOperationCost(type: 'llm' | 'image' | 'audio', mode: QualityMode = QualityMode.STANDARD): number {
        const costs = MODE_COSTS[mode];
        switch (type) {
            case 'llm': return costs.llmPerScene;
            case 'image': return costs.imagePerScene;
            case 'audio': return costs.audioPerScene;
            default: return 0;
        }
    }
}
