 ...
 */
export async function detectAndTrimSilence(
  inputPath: string,
  outputPath: string,
  threshold = '-40dB',
  duration = '0.1'
  ...
  try {
    const { stdout: out, stderr: err } = await execAsync(
      `ffmpeg -i "${inputPath}" -af "silencedetect=noise=${threshold}:d=${duration}" -f null -`
    )
    stdout = out
    stderr = err
  } catch (error: any) {
  ...
  try {
    ...
    try {
      ...
      totalDurationSec = Number.parseFloat(probeResult.stdout.trim())
    } catch {
      // fallback
    ...
    const silenceStarts = [...output.matchAll(/silence_start: ([\d.\-]+)/g)].map((m) => Number.parseFloat(m[1]))
    const silenceEnds = [...output.matchAll(/silence_end: ([\d.\-]+)/g)].map((m) => Number.parseFloat(m[1]))
