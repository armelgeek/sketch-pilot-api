import { WordTiming } from '../services/audio';
...
export class TimingMapper {
    ...
    static mapScenes(
        ...
        for (let sIdx = 0; sIdx < sceneNarrations.length; sIdx++) {
            const scene = sceneNarrations[sIdx];
        ...
        for (let j = 0; j < results.length; j++) {
            ...
            if (results[j].wordTimings.length > 0) {
                ...
                if (j < results.length - 1) {
                    const nextSceneStart = results[j + 1].wordTimings.length > 0
