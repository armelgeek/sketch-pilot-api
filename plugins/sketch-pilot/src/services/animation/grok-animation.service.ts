...
export class GrokAnimationService implements AnimationService {
     ...
     */
    async animateImage(imagePath: string, prompt: string, duration: number, outputPath: string, aspectRatio?: string): Promise<string> {
        console.log(`[GrokAnimation] Animating image (${duration}s): ${prompt.substring(0, 30)}...`);
                    ...
                    res.on('end', () => {
                        ...
                        try {
                            ...
                            if (videoUrl) {
                                ...
                                console.log(`[GrokAnimation] Polling status (Attempt ${attempts}): ${status || 'pending'} (HTTP ${res.statusCode})`);
                                setTimeout(checkStatus, 5000);
                            ...
                            console.error(`[GrokAnimation] JSON Parse error during polling:`, e);
                            setTimeout(checkStatus, 5000);
                        }
                    });
                });
        ...
        return new Promise((resolve, reject) => {
            https.get(url, (res) => {
                if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                    return this.downloadVideo(res.headers.location, outputPath, redirects + 1).then(resolve).catch(reject);
                }
