...
import { TextOverlayConfig } from '../../types/video-script.types';
...
export class AnimatedCaptionRenderer {
    ...
    private fetchWithRedirects(
        ...
        return new Promise((resolve, reject) => {
            const attempt = (currentUrl: string, remaining: number) => {
                ...
                mod.get(currentUrl, { headers: options.headers ?? {} }, (res) => {
                    if (
                        remaining > 0 &&
                        res.statusCode &&
                        res.statusCode >= 300 &&
    ...
    private async downloadFontAsBase64(url: string): Promise<string | null> {
        try {
            ...
            console.warn(`[AnimatedCaptionRenderer] Failed to download font from ${url}: ${e.message}`);
            return null;
        }
    }
    ...
    private getLayoutForPage(page: WordTiming[]): any[] {
        ...
        for (const line of lines) {
            const lineWidth = line[line.length - 1].x + line[line.length - 1].width;
    ...
    private buildFontDefs(): string {
        ...
        const formatHint = mime === 'font/woff2' ? 'woff2'
            : mime === 'font/woff' ? 'woff'
            : mime === 'font/opentype' ? 'opentype'
            : 'truetype';
        return `<defs><style>
@font-face {
  ...
  src: url('data:${mime};base64,${this.embeddedFontBase64}') format('${formatHint}');
    ...
    private renderSVGFrame(layout: any[], activeIndex: number, progress: number, prevLayoutIndex: number): string {
        ...
        let neonFilterDef = '';
        if (style === 'neon') {
            neonFilterDef = `<defs><filter id="neon-glow" x="-40%" y="-40%" width="180%" height="180%">` +
            ...
            if (style === 'colored-words' || style === 'scaling-words') {
                if (isActive) {
                    ...
                    if (style === 'scaling-words') {
                        scale = 1.0 + (0.2 * this.easeOutElastic(Math.min(1, progress * 4)));
                    }
                } else if (isPast) {
                    color = 'white';
                } else {
