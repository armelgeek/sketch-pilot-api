...
async function runPhase2Demo() {
  ...
  const topic =
    "Comment l'efficacité personnelle impacte le monde global (Utilise des métaphores comme globe-head et bulles de pensée)"
  ...
  result.script.scenes.forEach((scene: any, index: number) => {
    ...
    console.log(`Prompt Preview: ${scene.imagePrompt.slice(0, 150)}...`)
  })
