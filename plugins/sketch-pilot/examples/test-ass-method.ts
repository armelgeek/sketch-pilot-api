...
import { AssCaptionService, AssCaptionStyle, WordTiming } from '../src/services/video/ass-caption.service';
...
async function renderCaptionVideo(
  ...
  const totalMs    = words[words.length - 1].startMs + words[words.length - 1].durationMs;
  ...
  return new Promise((resolve, reject) => {
    ffmpeg()
      ...
      .save(videoPath)
      .on('end',   () => { console.log(`✅  [${style}] ${width}x${height} → ${videoPath}`); resolve(); })
      .on('error', (err: Error) => { console.error(`❌  [${style}] ${err.message}`); reject(err); });
  });
}
...
async function main() {
  ...
  for (const ar of ASPECT_RATIOS) {
    for (const style of STYLES) {
      ...
      const customConfig = ar.id === 'portrait' ? {
        style,
