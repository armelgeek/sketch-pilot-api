...
import { LayoutCatalog } from '../src/core/layout-catalog';
...
import { VideoScript } from '../src/types/video-script.types';
...
async function runFullOptimizationDemo() {
    ...
    try {
        ...
        try {
            ...
            console.log(`   • Tokens sent: ~${Math.ceil(systemPrompt.length / 4)} (system) + ~${Math.ceil(userPrompt.length / 4)} (user)`);
    ...
    const costSummary = {
        ...
        'Full Option B (all 3 optimizations)': {
            ...
            images: 0.0250,  // 75% resolution + 30% WebP
            audio: 0.0100,
            total: 0.0352
        }
    };
    ...
    for (const [scenario, costs] of Object.entries(costSummary)) {
        ...
        console.log(`  Images:   ${costs.images.toFixed(4)} USD (${costs.images === 0 ? '—' : ((costs.images / 0.1500) * 100).toFixed(0)}% of baseline)`);
