...
for (const style of newStyles) {
  ...
  if (!result.success) {
    console.error(`  ✗ FAIL: ${style}`, result.error)
    process.exit(1)
  }
...
for (const style of newStyles) {
  ...
  if (!ass.includes('[Script Info]') || !ass.includes('Dialogue:')) {
    console.error(`  ✗ FAIL: ${style} produced invalid ASS output`)
    process.exit(1)
  }
...
console.log('\n✓ Test 3: Bounce uses spring-physics sub-segments')
{
  ...
  const count = ass.split('\n').filter((l: string) => l.startsWith('Dialogue:')).length
  if (count < 8) {
    console.error(`  ✗ FAIL: expected ≥8 events, got ${count}`)
    process.exit(1)
  }
...
console.log('\n✓ Test 4: Typewriter invisible/fade-in behaviour')
{
  ...
  const ass = svc.buildASSFile(WORDS)
  if (!ass.includes(String.raw`\alpha&HFF&`)) {
    console.error(String.raw`  ✗ FAIL: typewriter missing \alpha&HFF& for invisible words`)
    process.exit(1)
  }
  if (!ass.includes(String.raw`\fad(`)) {
    console.error(String.raw`  ✗ FAIL: typewriter missing \fad fade-in tag`)
    process.exit(1)
  }
  console.log(String.raw`  ✓ typewriter: \alpha&HFF& (invisible) and \fad (fade-in) confirmed`)
}
...
console.log('\n✓ Test 5: Neon glow border tags')
{
  ...
  const ass = svc.buildASSFile(WORDS)
  if (!ass.includes(String.raw`\blur`) || !ass.includes(String.raw`\3c`)) {
    console.error(String.raw`  ✗ FAIL: neon missing \blur or \3c glow tags`)
    process.exit(1)
  }
  console.log(String.raw`  ✓ neon: \3c (outline colour) and \blur (glow) tags confirmed`)
}
...
console.log('\n✓ Test 6: Custom highlightColor respected')
{
  ...
  if (!ass.includes('FF00FF')) {
    console.error('  ✗ FAIL: custom highlight colour not found in output')
    process.exit(1)
  }
