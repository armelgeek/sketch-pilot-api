...
async function demonstrateLowResImageGeneration() {
    ...
    const prompt = 'A simple stickman character standing confidently with arms crossed, sketchy black ink on white background';
    const systemPrompt = 'You are a stickman animation artist. Create minimalist, sketchy hand-drawn stickman characters on white backgrounds.';
    ...
    console.log(`Medium (60%):               $${costs.medium.toFixed(2)}  (${(100 - 100 * costs.medium / costs.high).toFixed(0)}% saving)`);
    console.log(`Low (40%):                  $${costs.low.toFixed(2)}  (${(100 - 100 * costs.low / costs.high).toFixed(0)}% saving) ✅`);
    console.log(`Ultra-low (20%):            $${costs['ultra-low'].toFixed(2)}  (${(100 - 100 * costs['ultra-low'] / costs.high).toFixed(0)}% saving)`);
    console.log();
