...
import { AudioServiceFactory, type AudioServiceConfig } from '../src/services/audio'
...
async function main() {
  ...
  console.log(
    `Google TTS:    ${results.googleTTS === null ? '⚠️  SKIPPED' : results.googleTTS ? '✅ PASS' : '❌ FAIL'}`
  )
  console.log(
    `ElevenLabs:    ${results.elevenlabs === null ? '⚠️  SKIPPED' : results.elevenlabs ? '✅ PASS' : '❌ FAIL'}`
  )
