...
async function loadDemoWordsFromAudio(audioPath: string): Promise<void> {
  ...
  try {
    ...
    DEMO_WORDS = result.wordTimings.map(w => ({
      ...w,
...
function calcFitFontSize(
  ...
  for (let fs = maxSize; fs >= minSize; fs -= 2) {
    ...
    const totalW = words.reduce((sum, w, i) =>
        sum + estimateWordWidthPx(w, fs) * (i === activeIdx && activeScale > 1.0 ? activeScale : 1.0), 0)
...
function defaultColors(style: AssCaptionStyle) {
  return {
    highlightColor: style === 'colored' ? '#FFE135'
                  : style === 'scaling' ? '#4ADE80'
                  : style === 'bounce'  ? '#FFE135'
                  : style === 'neon'    ? '#00FFFF'
                  : '#FFFFFF',
...
function buildPreviewSVG(opts: {
  ...
  const { width, height, style, wordsPerLine, fontSize,
          inactiveColor, highlightColor, pillColor } = opts;
  ...
  const getLineYFraction = (s: AssCaptionStyle): number => {
    ...
    if (s === 'bounce') return 0.50;
    if (s === 'scaling') return 0.50;
    if (s === 'neon') return 0.48; // Glow needs slight adjustment
    if (s === 'typewriter') return 0.50;
    return 0.50; // colored, karaoke, remotion
  };
  ...
  const effectiveWidths = wordWidths.map((w, i) =>
    isScalingStyle && i === activeIdx ? w * SCALING_SCALE : w);
  ...
  let wordsSvg = '';
  let curX = startX;
  const defsSvg = style === 'neon' ? `<defs><filter id="glow"><feGaussianBlur stdDeviation="${(fontSize * 0.08).toFixed(1)}" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>` : '';
  ...
  words.forEach((word, i) => {
    ...
    let fillColor = isActive
      ? (style === 'animated-background' ? '#FFFFFF' : highlightColor)
      : inactiveColor;
    ...
    const scale  = isScalingStyle && isActive ? SCALING_SCALE : 1.0;
...
async function main() {
  ...
  for (let row = 0; row < ROWS; row++) {
    ...
    for (let col = 0; col < COLS; col++) {
      ...
      const activeScale  = st.id === 'scaling' ? 1.3 : 1.0;
      const rawFontSize  = calcFitFontSize(previewWords, thumbW * 0.80, st.id, 16, 80, activeScale);
      ...
      const svg = buildPreviewSVG({
        ...
        style: st.id, wordsPerLine, fontSize,
        ...colors,
      });
      ...
      const labelBuf = await sharp(Buffer.from(buildLabelSVG(
        `${ar.label}  ·  ${st.label}`, thumbW, LABEL_H,
      ))).png().toBuffer();
      ...
      const rawFullFontSize = calcFitFontSize(fullWords, fullAR.width * 0.80, st.id, 16, 80, activeScale);
      const fullFontSize = ar.id === 'portrait'
        ? Math.max(16, Math.round((rawFullFontSize * 0.6) / 2) * 2)
        : rawFullFontSize;
      const fullSvg = buildPreviewSVG({
        width: fullAR.width, height: fullAR.height,
        style: st.id, wordsPerLine, fontSize: fullFontSize,
        ...colors,
      });
      ...
      console.log(`  ✅  ${ar.id}/${st.id}/preview.png  (${wordsPerLine} words/line, grid=${fontSize}px, full=${fullFontSize}px)`);
    }
    ...
    const rowBuf = await sharp({
      ...
      .composite(rowCells.map((buf, col) => ({
        input: buf,
        top:  0,
  ...
  for (const ar of FULL_ASPECT_RATIOS) {
    for (const st of STYLES) {
      ...
      console.log(`  📄  ${ar.id}/${st.id}/${st.id}.ass  (${svc.getResolvedWordsPerLine()} words/line, ${svc.getResolvedFontSize()}px)`);
    }
  }
  ...
  for (const ar of FULL_ASPECT_RATIOS) {
    for (const st of STYLES) {
      ...
      const totalMs  = DEMO_WORDS[DEMO_WORDS.length - 1].startMs +
                       DEMO_WORDS[DEMO_WORDS.length - 1].durationMs;
      ...
      await new Promise<void>((resolve, reject) => {
        ffmpeg()
          ...
          .save(videoPath)
          .on('end',   () => { console.log(`  ✅  ${ar.id}/${st.id}/${st.id}.mp4`); resolve(); })
          .on('error', (err: Error) => { console.error(`  ❌  ${ar.id}/${st.id}: ${err.message}`); reject(err); });
      });
    }
  }
