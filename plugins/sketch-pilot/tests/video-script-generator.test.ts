...
;(async () => {
  ...
  structure.scenes.forEach((s: any, idx: number) => {
    ...
    if (
      ctx &&
      !['quick-list', 'transition', 'story', 'explanation', 'detailed-breakdown', 'conclusion'].includes(ctx)
    ) {
  ...
  const lastNarr = structure.scenes.at(-1).narration.toLowerCase()
  ...
  structure.scenes.forEach((s: any, idx: number) => {
    ...
    const answers = (text.match(/\b(because|so|therefore|the answer|which means|thus|hence|here's why)\b/i) || [])
      .length
  ...
  if (
    structure.scenes[1].narration &&
    structure.scenes[1].narration.trim().toLowerCase() === structure.scenes[0].narration.trim().toLowerCase()
  ) {
  ...
  const cleaned =
    (gen as any).postProcess && typeof (gen as any).postProcess === 'function'
