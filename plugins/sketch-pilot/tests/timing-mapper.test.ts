...
async function testTimingMapper() {
  ...
  if (results[0].wordTimings.length !== 7)
    throw new Error(`Scene 1 should have 7 words, got ${results[0].wordTimings.length}`)
  if (results[0].start !== 0.1) throw new Error(`Scene 1 start should be 0.1, got ${results[0].start}`)
  if (results[0].end !== 3) throw new Error(`Scene 1 end should be 3.0, got ${results[0].end}`)

  // Scene 2: "And this is the second one, with more text." (9 words)
  if (results[1].wordTimings.length !== 9)
    throw new Error(`Scene 2 should have 9 words, got ${results[1].wordTimings.length}`)
