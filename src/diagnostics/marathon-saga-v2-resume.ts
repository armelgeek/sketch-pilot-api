...
async function resumeMarathonSaga() {
  const apiKey = process.env.OPENAI_API_KEY
  if (!apiKey) return

  const seriesId = 'chronos-breach-v2-1775989526462' // Re-use ID
  const totalArcs = 10
  const epsPerArc = 2

  // READ LAST METADATA
  const lastMetadataPath = path.join(process.cwd(), 'storage', 'narrations', seriesId, 'arc_5_ep_2_METADATA.json')
  const lastMetadata = JSON.parse(fs.readFileSync(lastMetadataPath, 'utf8'))

  // RECONSTRUCT BIBLE
  let context: any = {
    seriesId,
    episodeNumber: 10, // Ep 10 was the last successful one
  ...
  // HYDRATE WITH LAST STEP
  context = (SeriesVideoGenerator as any).updateContext(lastMetadata, context)
  ...
  for (let arc = 6; arc <= totalArcs; arc++) {
    for (let ep = 1; ep <= epsPerArc; ep++) {
      ...
      if (d1.error) {
        console.error('Error Ep1:', d1.error)
        break
      }
      ...
      if (d2.error) {
        console.error('Error Ep2:', d2.error)
        break
      }
