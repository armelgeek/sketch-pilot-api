...
export class VideoGenerationService {
  ...
  private async buildEngine(
    ...
    return new NanoBananaEngine(
      ...
      undefined, // transcriptionConfig
      {
        ...
        characterModelId: (options.characterModelId as string) || undefined,
  ...
  async generateCharacterImage(input: {
    ...
    const { prompt, baseModelId, outputDir, videoId, referenceImages = [] } = input
    ...
    const scene: any = {
      id: 'char-gen',
      imagePrompt: `ABSOLUTE CHARACTER IDENTITY LOCK: ${prompt}. The character MUST be 100% recognizable as the same person in the reference images.`,
      locationId: 'studio'
    }
