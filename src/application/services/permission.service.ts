...
export class PermissionService {
  ...
  // Retourne un rôle unique avec ses utilisateurs et permissions
  async getRoleWithDetailsById(roleId: string) {
    const result = await db
      .select({
        roleId: roles.id,
        roleName: roles.name,
    ...
    // Regroupement comme dans getAllRolesWithDetails
    const role = {
      id: result[0].roleId,
    ...
    result.forEach((row) => {
      if (row.userId && !role.users.has(row.userId)) {
        role.users.set(row.userId, {
          ...
          name: row.userName,
          email: row.userEmail,
          lastLoginAt: row.userLastLoginAt ? new Date(row.userLastLoginAt).toISOString() : null
        })
      }
      if (row.resourceId && !role.resources.has(row.resourceId)) {
        role.resources.set(row.resourceId, {
